    ...
    <div class="art-top" v-show="headerShow" :class="{'top-hide':isShow}" @mouseenter="onMouseover()"  @mouseleave="onMouseout()" > 
          <div class="top-left">
            <a href="#" id="stu-full-screen" title="全屏" v-if="isRefer != 'refer'" @click="showFull">
                <svg class="icon" aria-hghidden="true">
            ...
            <a href="javascript:void(0);" id="btnCopy" title="选择文字" @click="btnCopy" :class="isHua ? 'cur' : ''">
            ...
            <!-- <a href="javascript:void(0);" title="自定义线型" style="display:inline-block;" :class=" isAddPeg ? 'cur' : ''" @click="addPeg">
        ...
        <a class="art-top-btn" @click="showDialog()" ><svg aria-hidden="true" class="icon"><use xlink:href="#icon-open"></use></svg></a>
</div>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState(['headerShow','isDrag',"isAddPeg","isHua",'isRefer','otherArcInfo','iframSrcObj']),
            ...mapState('pdfData', ["initPageNo","originalWidth"]),
        },
        methods:{
            ...mapMutations(["setCompareIfrShow","showArtTop","isFullBoxShowFun","isHuaFun","isDragFun","setCenterStyle","setLeftStyle","setTopStyle","setRightStyle","setLeftIconStyle","setRightIconStyle","setTaskInfo","setCompare"]),
           ...
           gotoPage(){
                ...
                if(page > this.totalPage){
                    layer.msg("请输入合法页码",{skin:" layui-layer tip-orange "});
                    $("#pageIndex").val(this.initPageNo);
                    return false;
                }
                this.setPageNoF(page);
            },
            addPeg(){
               this.isFullBoxShowFun([false,false,true]);
            },
            showFull(){
            ...
            // 拖拽阅读
            btnCopyCancel(){
                this.isFullBoxShowFun([true,false,false]);
            },
            // 划词阅读
            btnCopy(){
            ...
            showReferFull(){
               var href = window.InitData.ApplicationPath + 'article/pdfonline?filename=' + window.InitData.fileName + '&tablename=' + window.InitData.tableName + '&dbcode=' + window.InitData.dbCode + '&filesourcetype='+window.InitData.FileSourceType + '&topic=' + window.InitData.Topic + '&appId=' + window.InitData.AppId + '&pageType=POPPAGE#/pdf';
                window.open(href);
            },
            // 对比阅读
            compareReadFun(){
